...
import pandas as pd
import logging
logging.basicConfig(level=logging.DEBUG, filename='/var/log/flaskapp/flaskapp-debug.log', filemode='w')
...
class Users(Resource):
    def __init__(self):
        self.data = pd.read_csv('/var/lib/flaskapp/kullanicilar.csv')
    ...
    def post(self):
        ...
        self.data.to_csv('/var/lib/flaskapp/kullanicilar.csv', index=False)
...
class Cities(Resource):
    def get(self):
        data = pd.read_csv('/var/lib/flaskapp/kullanicilar.csv', usecols=[2])
        data = data.to_dict('records')
        return {'data': data}, 200

class Name(Resource):
    def get(self, name):
        data = pd.read_csv('/var/lib/flaskapp/kullanicilar.csv')
        data = data.to_dict('records')
        for entry in data:
            if entry['name'] == name:
